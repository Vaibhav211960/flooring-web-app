...
export default function BuyAll() {
  ...
  // Banner Message Logic (Upsell Scheme)
  ...
  const handlePlaceOrder = async () => {
    ...
    setTimeout(() => {
      ...
      toast({
        title: "ORDER PLACED SUCCESSFULLY", // Uppercase for industrial feel
        description: `Reference ID: FLR-${Math.floor(Math.random() * 90000)}`,
        // Refined Black Styling
        className:
          "bg-stone-950 border border-stone-800 text-white rounded-xl shadow-2xl p-6",
      });
      ...
      <main className="flex-grow py-12">
        <div className="container max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8 space-y-8">
            {/* Upsell Banner */}
            <div
              className={`p-4 rounded-xl border flex items-center justify-between transition-all duration-500 ${upsellMessage.color}`}
            >
            ...
            <div className="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden">
              <div className="p-6 border-b border-stone-100 flex items-center gap-3">
                ...
                <h2 className="font-serif font-bold text-lg">
                  Project Inventory ({cartItems.length})
                </h2>
              </div>
                ...
                {cartItems.map((item) => (
                  <div key={item._id} className="p-6 flex items-center gap-6">
                    ...
                    <div className="flex-1">
                      <h4 className="font-bold text-sm text-stone-800 uppercase tracking-tight">
          ...
          <div className="lg:col-span-4">
            <div className="bg-white rounded-2xl border border-stone-200 shadow-xl p-8 sticky top-28">
              ...
              <div className="space-y-4 text-sm border-b border-stone-100 pb-6">
                ...
                {discountData.p > 0 && (
                  <div className="flex justify-between text-emerald-600 font-bold">
                    ...
                    <span className="font-mono">
                      - ₹{discountData.amt.toLocaleString()}
                    </span>
                  </div>
                )}
              ...
              <div className="py-6 flex justify-between items-baseline">
                <span className="font-bold text-stone-900 uppercase text-[10px] tracking-widest">
                  Net Payable
                </span>
                <span className="text-3xl font-mono font-bold text-amber-800">
              ...
              <Button
                ...
                className="w-full h-14 bg-stone-900 hover:bg-stone-800 text-white rounded-none font-bold uppercase tracking-widest transition-all active:scale-95"
              >
                {isPlacingOrder ? (
...
function InputField({ label, error, ...props }) {
  return (
    <div className="space-y-2 flex flex-col">
      ...
      />
      {error && (
        <p className="text-[10px] text-red-600 font-bold mt-1 ml-1 uppercase tracking-tighter">
          ! {error}
        </p>
      )}
    </div>
