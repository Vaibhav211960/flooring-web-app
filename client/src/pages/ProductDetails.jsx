...
import { 
  ShoppingCart, CreditCard, ChevronRight, Home, 
  ShieldCheck, Zap, Ruler, Layers, Box, Info, Loader2, Warehouse, Palette
} from "lucide-react";
    ...
    return (
      <div className="min-h-screen bg-stone-50 flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-20 text-center">
          ...
          <Link to="/products" className="text-amber-700 underline uppercase tracking-[0.2em] text-[10px] font-bold">
    ...
    <div className="min-h-screen bg-stone-50 flex flex-col text-stone-900">
      ...
      <nav className="border-b border-stone-200 bg-white/50 backdrop-blur-sm sticky top-[64px] z-20">
        <div className="container max-w-7xl mx-auto px-6 py-3 flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-stone-400">
          <Link to="/" className="hover:text-amber-700 flex items-center gap-1 transition-colors"><Home className="h-3 w-3" /> Home</Link>
          ...
          <div className="lg:col-span-7 space-y-12">
            ...
            <section className="space-y-6">
              <div className="inline-flex items-center gap-2 text-amber-700">
                ...
                <h3 className="text-xs font-bold uppercase tracking-widest">Material Narrative</h3>
              </div>
              ...
              <p className="text-stone-600 leading-relaxed max-w-2xl">
                This {product.woodType} selection is processed with a {product.finish} finish, 
                ensuring a high-end architectural look. Categorized under ID: {product.subCategoryId.name}, 
                this product meets our strict industrial standards for {product.unit} distribution.
              </p>
            </section>
          </div>
          ...
          <div className="lg:col-span-5 flex flex-col gap-10">
            <div className="space-y-2">
              ...
              <div className="flex items-baseline gap-2 pt-2">
                <span className="text-3xl font-medium text-stone-900">₹{product.price}</span>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-lg bg-white border border-stone-200">
                <div className="flex items-center gap-2 text-stone-400 mb-1">
                  <Layers size={14} /><span className="text-[9px] uppercase tracking-widest font-bold">Thickness</span>
                </div>
                <span className="text-sm font-semibold text-stone-800">{product.thicknessMM} mm</span>
              </div>
              <div className="p-4 rounded-lg bg-white border border-stone-200">
                <div className="flex items-center gap-2 text-stone-400 mb-1">
                  <Box size={14} /><span className="text-[9px] uppercase tracking-widest font-bold">Finish</span>
                </div>
                <span className="text-sm font-semibold text-stone-800">{product.finish}</span>
              </div>
              <div className="p-4 rounded-lg bg-white border border-stone-200">
                <div className="flex items-center gap-2 text-stone-400 mb-1">
                  <Palette size={14} /><span className="text-[9px] uppercase tracking-widest font-bold">Color</span>
                </div>
                <span className="text-sm font-semibold text-stone-800">{product.color || "Natural"}</span>
              </div>
              <div className="p-4 rounded-lg bg-white border border-stone-200">
                <div className="flex items-center gap-2 text-stone-400 mb-1">
                  <Warehouse size={14} /><span className="text-[9px] uppercase tracking-widest font-bold">Availability</span>
                </div>
            ...
            <div className="space-y-6 pt-6 border-t border-stone-200">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center bg-white border border-stone-300 rounded-md shadow-sm">
                  <button onClick={() => setQty(Math.max(1, qty - 1))} className="px-4 py-2 hover:bg-stone-50 text-stone-400">-</button>
                  <span className="px-6 py-2 font-bold text-stone-900 border-x border-stone-100 min-w-[3rem] text-center">{qty}</span>
                  <button onClick={() => setQty(qty + 1)} className="px-4 py-2 hover:bg-stone-50 text-stone-400">+</button>
                </div>
              </div>
...
>
  <CreditCard size={18} /> 
  {localStorage.getItem("token") ? "Buy Now with Razorpay" : "Login to Purchase"}
</Button>
                <Button variant="outline" className="w-full h-14 border-stone-300 text-stone-700 hover:bg-amber-50 hover:border-amber-200 text-sm font-bold uppercase tracking-widest rounded-md">
              ...
              <div className="flex items-center justify-center gap-6 py-2">
                <div className="flex items-center gap-2 text-[10px] text-stone-400 uppercase font-medium">
                  <ShieldCheck size={14} className="text-emerald-500" /> Secured Payment
                </div>
                <div className="flex items-center gap-2 text-[10px] text-stone-400 uppercase font-medium">
