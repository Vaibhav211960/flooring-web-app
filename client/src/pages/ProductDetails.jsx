...
import {
  ShoppingCart, CreditCard, ChevronRight, Home, ShieldCheck,
  Zap, Layers, Box, Info, Loader2, Warehouse, Palette, Star, Send, User
} from "lucide-react";
...
export default function ProductDetails() {
  ...
  const [reviews, setReviews] = useState([
    { id: 1, user: "Ashok", comment: "The quality of the wood is impressive, perfect finish.", date: "2 mins ago" },
    { id: 2, user: "Ayush", comment: "Great value for money. Looks even better in person.", date: "1 hour ago" }
  ]);
  ...
  if (isLoading) return (
    <div className="min-h-screen bg-stone-50 flex flex-col items-center justify-center">
      ...
      <p className="mt-4 text-stone-500 font-serif italic tracking-wide">Syncing Specifications...</p>
    </div>
  );
  ...
  if (error || !product) return (
    <div className="min-h-screen bg-stone-50 flex flex-col italic items-center justify-center">
        ...
        <Link to="/products" className="text-amber-700 underline uppercase tracking-widest text-[10px] font-bold">Return to Collections</Link>
    </div>
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
          ...
          <div className="lg:col-span-7 space-y-12">
            <div className="aspect-square bg-white rounded-xl border border-stone-200 overflow-hidden shadow-sm">
              <img src={product.image} className="w-full h-full object-cover transition-transform duration-700" alt={product.name} />
            </div>

            <section className="space-y-8">
              <div className="flex items-center gap-2 text-amber-700 border-b border-stone-200 pb-2">
                <Info size={16} />
                <h3 className="text-[10px] font-bold uppercase tracking-[0.3em]">Material Narrative</h3>
              </div>
              <p className="font-serif text-3xl text-stone-800 leading-tight italic">"{product.description}"</p>
              
              {/* --- DECENT REVIEW SECTION --- */}
              <div className="pt-4 space-y-6">
                <div className="flex items-center justify-between">
                  <h4 className="text-[10px] font-bold uppercase tracking-widest text-stone-400">Client Feedback ({reviews.length})</h4>
                ...
                <div className="space-y-4">
                  {reviews.map((rev) => (
                    <div key={rev.id} className="bg-white/60 p-5 rounded-xl border border-stone-100 flex gap-4 items-start transition-all hover:border-stone-200">
                      <div className="p-2 bg-stone-100 rounded-full text-stone-400">
                        <User size={16} />
                      </div>
                      <div className="flex-1">
                        <div className="flex justify-between items-center mb-1">
                          <span className="text-xs font-bold text-stone-800 uppercase tracking-wide">{rev.user}</span>
                          <span className="text-[9px] text-stone-400 font-medium uppercase tracking-tighter">{rev.date}</span>
                        </div>
                        <p className="text-sm text-stone-600 leading-relaxed italic">"{rev.comment}"</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-6 pt-6 border-t border-stone-200">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center bg-white border border-stone-300 rounded-md shadow-sm">
                  <button onClick={() => setQty(Math.max(1, qty - 1))} className="px-4 py-2 hover:bg-stone-50 transition-colors">-</button>
                  <span className="px-6 py-2 font-bold text-stone-900 border-x border-stone-100 min-w-[3rem] text-center">{qty}</span>
                  <button onClick={() => setQty(qty + 1)} className="px-4 py-2 hover:bg-stone-50 transition-colors">+</button>
                </div>
              </div>

              <div className="flex flex-col gap-3">
                <Button onClick={handleBuyNow} className="w-full h-14 bg-stone-900 text-white hover:bg-stone-800 text-[17px] font-bold uppercase tracking-[0.2em] rounded-xl flex items-center justify-center gap-3 transition-all duration-300 border-none shadow-sm">
              ...
              <div className="flex items-center justify-center gap-6 py-2">
                <div className="flex items-center gap-2 text-[10px] text-stone-400 uppercase font-medium font-bold"><ShieldCheck size={14} className="text-emerald-500" /> Secured Payment</div>
                <div className="flex items-center gap-2 text-[10px] text-stone-400 uppercase font-medium font-bold"><Zap size={14} className="text-amber-500" /> Fast Shipping</div>
              </div>
            </div>
          </div>
        </div>
