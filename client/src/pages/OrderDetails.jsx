...
import {
  ...
  ArrowLeft, CheckCircle2, MapPin, User, Phone, Hash, Loader2, Star,
  Send, Lock, Pencil, Trash2, X, Check, MessageSquare, XCircle, AlertTriangle,
} from "lucide-react";
...
// ─── Cancellation Banner ───────────────────────────────────────────────────
function CancellationBanner({ order }) {
  return (
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-stone-500 leading-relaxed max-w-xl">
              Your order <span className="font-bold text-stone-700">#{order._id.slice(-6).toUpperCase()}</span> has been cancelled by our team.
          ...
          {[
            {
              ...
              desc: order.paymentMode === "COD"
                ? "No charge was made for COD orders."
...
function ProductFeedbackPanel({ productId, productName, orderDelivered }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const reviewRes = await axios.get(`http://localhost:5000/api/feedback/product/${productId}`);
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      setReviewText(""); setRating(5); setIsEligible(false);
  ...
  const handleEditSave = async (reviewId) => {
    ...
    try {
      setIsSaving(true);
      const res = await axios.put(`http://localhost:5000/api/feedback/${reviewId}`,
    ...
    } catch (err) { toast.error(err.response?.data?.message || "Could not update review."); }
    ...
    <div className="border-t border-stone-100 pt-6 mt-2 space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-amber-700">Reviews for {productName}</p>
        </div>
        {avgRating && (
          <div className="flex items-center gap-1.5 bg-stone-50 border border-stone-200 rounded-lg px-3 py-1.5">
            ...
            <span className="text-[9px] text-stone-400 font-medium">({reviews.length} {reviews.length === 1 ? "review" : "reviews"})</span>
          </div>
        )}
      </div>
      ...
      {orderDelivered && token && isEligible && (
        <div className="bg-amber-50/60 border border-amber-100 p-5 rounded-xl space-y-4">
          ...
          <form onSubmit={handleReviewSubmit} className="relative">
            ...
            <button type="submit" disabled={isSubmitting} className="absolute bottom-3 right-3 bg-stone-900 text-amber-500 p-2.5 rounded-lg hover:bg-stone-800 transition-all disabled:opacity-50">
      ...
      {orderDelivered && token && !isEligible && reviews.length === 0 && !reviewsLoading && (
        <div className="flex items-center gap-3 p-4 bg-stone-50 rounded-xl border border-stone-200">
          ...
          <p className="text-xs text-stone-500 italic">No reviews yet. You have already reviewed this product.</p>
        </div>
      )}
      ...
      {!orderDelivered && token && (
        <div className="flex items-center gap-3 p-4 bg-stone-50 rounded-xl border border-stone-200">
          <Lock size={13} className="text-stone-400 shrink-0" />
          <p className="text-xs text-stone-500 italic">You can leave a review once this order is delivered.</p>
        </div>
      )}

      {reviewsLoading ? (
        <div className="flex items-center gap-2 py-4 text-stone-400"><Loader2 size={14} className="animate-spin" /><span className="text-xs">Loading reviews...</span></div>
      ) : reviews.length === 0 ? (
        ...
        <div className="space-y-3">
          {visibleReviews.map((rev) => {
            ...
            return (
              <div key={rev._id} className={`bg-white p-5 rounded-xl border shadow-sm transition-all ${isEditing ? "border-amber-300" : "border-stone-100 hover:border-stone-200"}`}>
                {isEditing ? (
                  <div className="space-y-3">
                    <div className="flex items-center gap-2 text-amber-700"><Pencil size={11} /><span className="text-[10px] font-bold uppercase tracking-widest">Editing your review</span></div>
                    <div className="flex gap-1">
                      {[1,2,3,4,5].map((star) => (
                        <button type="button" key={star} onClick={() => setEditRating(star)}>
                          <Star size={17} fill={star <= editRating ? "#b45309" : "none"} className={star <= editRating ? "text-amber-700" : "text-stone-300"} />
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="flex gap-2 justify-end">
                      <button onClick={handleEditCancel} className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest text-stone-500 bg-stone-100 rounded-lg hover:bg-stone-200 transition-all"><X size={11} /> Cancel</button>
                      <button onClick={() => handleEditSave(rev._id)} disabled={isSaving} className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest text-white bg-stone-900 rounded-lg hover:bg-stone-800 transition-all disabled:opacity-50">
                  ...
                  <>
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-xs font-bold text-stone-900 flex items-center gap-1.5">{rev.userId?.userName || "Anonymous"}<CheckCircle2 size={10} className="text-emerald-500" /></p>
                          <div className="flex gap-0.5 mt-0.5">
                            {[...Array(5)].map((_, i) => <Star key={i} size={9} fill={i < rev.rating ? "#f59e0b" : "none"} className={i < rev.rating ? "text-amber-500" : "text-stone-200"} />)}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] text-stone-400">{new Date(rev.createdAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</span>
                        {isOwner && (
                          <div className="flex items-center gap-1">
                            <button onClick={() => handleEditStart(rev)} className="p-1.5 text-stone-400 hover:text-amber-700 hover:bg-amber-50 rounded-lg transition-all"><Pencil size={11} /></button>
                            <button onClick={() => handleDelete(rev._id)} className="p-1.5 text-stone-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all"><Trash2 size={11} /></button>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          {reviews.length > 2 && (
            <button onClick={() => setShowAll((v) => !v)} className="text-[10px] font-bold uppercase tracking-widest text-amber-700 hover:text-amber-800 transition-colors py-1">
              {showAll ? "Show Less" : `Show ${reviews.length - 2} More Review${reviews.length - 2 > 1 ? "s" : ""}`}
            </button>
          )}
        </div>
      )}
...
export default function OrderDetails() {
  ...
  useEffect(() => {
    ...
    if (hasCheckoutData) {
      clearCart().catch((err) =>
        console.error("Post-order cart clear failed:", err?.response?.status, err?.response?.data)
      );
    }
  }, []);
  ...
  const generateInvoice = () => {
    try {
      ...
      autoTable(doc, {
        ...
        body: order.items.map((item, i) => [i + 1, { content: item.productName.toUpperCase(), styles: { fontStyle: "bold" } }, item.units, `INR ${Number(item.pricePerUnit).toLocaleString("en-IN")}`, `INR ${Number(item.totalAmount).toLocaleString("en-IN")}`]),
        theme: "grid",
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col bg-stone-50"><Navbar />
      ...
      <section className={`${isCancelled ? "bg-red-950" : "bg-stone-900"} text-stone-50 border-b border-amber-900/20 transition-colors`}>
        <div className="container max-w-7xl mx-auto px-6 py-16 md:py-24">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            {/* Hide invoice download for cancelled orders */}
            {!isCancelled && (
      ...
      <main className="flex-grow py-12 md:py-16">
        <div className="container max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            {/* ── CANCELLATION BANNER (replaces delivery progress when cancelled) ── */}
            {isCancelled ? (
              <CancellationBanner order={order} />
            ) : (
              <div className="bg-white rounded-2xl border border-stone-200 p-8 shadow-sm">
                <div className="px-0 pb-6 border-b border-stone-100 mb-8">
                  <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-amber-700 flex items-center gap-2"><Truck size={14} /> Delivery Progress</p>
                </div>
            ...
            <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden shadow-sm">
              <div className="px-8 py-5 border-b border-stone-100 flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">Tx ID: {order.paymentId?.slice(-8) || "—"}</span>
              </div>
              <div className="divide-y divide-stone-100">
                {order.items.map((item, idx) => (
                  <div key={idx} className="p-8">
                    ...
                    {/* Hide feedback panel for cancelled orders */}
                    {!isCancelled && (
                      <ProductFeedbackPanel productId={item.productId} productName={item.productName} orderDelivered={isDelivered} />
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden">
              <div className="px-6 py-5 border-b border-stone-100">
                <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-amber-700 flex items-center gap-2"><MapPin size={14} /> Destination Details</p>
              </div>
            ...
            <div className={`${isCancelled ? "bg-red-950" : "bg-stone-900"} text-stone-50 rounded-2xl shadow-xl overflow-hidden transition-colors`}>
              <div className="px-6 py-5 border-b border-stone-800">
                <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-amber-500 flex items-center gap-2"><CreditCard size={14} /> Financial Summary</p>
              </div>
              <div className="p-6 space-y-4">
                <div className="flex justify-between text-sm"><span className="text-stone-400 font-medium tracking-wide">Gross Bill</span><span className={`font-bold ${isCancelled ? "line-through text-stone-500" : ""}`}>₹{order.netBill.toLocaleString()}</span></div>
                <div className="flex justify-between text-sm">
                  <span className="text-stone-400 font-medium tracking-wide">Status</span>
