...
export default function OrderDetails() {
  ...
  // ── Fetch order + reviews + eligibility ─────────────────────
  useEffect(() => {
      ...
      try {
        ...
        if (token) {
          const eligRes = await axios.get(
            `http://localhost:5000/api/feedback/verify-eligibility/order/${orderId}`,
            { headers }
          );
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        ...
        { orderId, comment: reviewText, rating },
      ...
      <section className="bg-stone-900 text-stone-50 border-b border-amber-900/20">
        <div className="container max-w-7xl mx-auto px-6 py-16 md:py-24">
          <nav className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-stone-400 font-bold mb-8">
            ...
            <Link to="/order-history" className="hover:text-white transition-colors">
              History
            </Link>
            <ChevronRight className="h-3 w-3 text-stone-700" />
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <h1 className="font-serif text-4xl md:text-5xl font-bold leading-tight mb-2">
                Order <span className="italic text-amber-400">#{order._id.slice(-6)}</span>
              </h1>
            ...
            <div className="bg-white rounded-2xl border border-stone-200 p-8 shadow-sm">
              ...
              <div className="flex items-center justify-between relative">
                ...
                <StatusStep
                  ...
                  label="In Transit"
                  sub="Shipping"
                  active={order.orderStatus === "arriving" || order.orderStatus === "delivered"}
                />
                <StatusStep
              ...
              {isEligible ? (
                <div className="bg-white border border-amber-100 p-6 rounded-xl shadow-sm space-y-4">
                  ...
                  <form onSubmit={handleReviewSubmit} className="relative">
                    <textarea
                      ...
                      placeholder="Share your thoughts on delivery, packaging, and overall experience..."
                ...
                <div className="flex items-center gap-3 p-4 bg-stone-50 rounded-xl border border-stone-200">
                  ...
                  <p className="text-xs text-stone-500 italic">
                    Only verified buyers who have received this order can post reviews.
                  </p>
                </div>
              )}
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-stone-600 leading-relaxed uppercase italic">
                      ...
                      {order.shippingAddress.landmark && (
                        <><br />Landmark: {order.shippingAddress.landmark}</>
                      )}
                    </p>
                  </div>
                </div>
